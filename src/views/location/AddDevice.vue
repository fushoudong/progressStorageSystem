...
export default {
  ...
  methods: {
    ...
    close() {
      this.$emit("update:addmodalvisibale", false);
    },
    determine() {
      var self = this;
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (!self.fromDataCopy.image) {
            myalert.fail("请上传图片");
            return false;
          } else {
            self.$emit("update:addmodalvisibale", false);
            self.$emit("finish", self.fromDataCopy);
          }
        }
      });
    }
